...
class Procedimento extends CI_Controller {
    ...
    public function cadastrar() {
	   ...
	   $this->form_validation->set_error_delimiters('<div class="alert alert-danger" role="alert">', '</div>');
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $data['idApp_ProcedimentoCli'] = $this->Procedimento_model->set_procedimento($data['query']);
    ...
    public function alterar($id = FALSE) {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $data['anterior'] = $this->Procedimento_model->get_procedimento($data['query']['idApp_ProcedimentoCli']);
            $data['campos'] = array_keys($data['query']);

            $data['auditoriaitem'] = $this->basico->set_log($data['anterior'], $data['query'], $data['campos'], $data['query']['idApp_ProcedimentoCli'], TRUE);

            if ($data['auditoriaitem'] && $this->Procedimento_model->update_procedimento($data['query'], $data['query']['idApp_ProcedimentoCli']) === FALSE) {
                $data['msg'] = '?m=2';
                redirect(base_url() . 'procedimento/form_procedimento/' . $data['query']['idApp_ProcedimentoCli'] . $data['msg']);
                exit();
            } else {
