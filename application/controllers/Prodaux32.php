...
class Prodaux32 extends CI_Controller {
    ...
    public function cadastrar3($tabela = NULL) {
        ...
        $data['form_open_path'] = 'prodaux32/cadastrar3';
        $data['readonly'] = '';
        $data['disabled'] = '';
        $data['panel'] = 'primary';
        $data['metodo'] = 1;
        ...
        $data['list'] = $this->load->view('prodaux3/list_prodaux33', $data, TRUE);

        #run form validation
        if ($this->form_validation->run() === FALSE) {
            $this->load->view('prodaux3/pesq_prodaux33', $data);
        } else {
           ...
           # $data['query']['ValorVenda'] = str_replace(',','.',str_replace('.','',$data['query']['ValorVenda']));
            ...
            if ($data['idTab_Prodaux3'] === FALSE) {
                ...
                $this->load->view('prodaux3/cadastrar3', $data);
            } else {
                ...
                redirect(base_url() . 'prodaux32/cadastrar3' . $data['msg']);
                exit();
            }
        }
    ...
    public function alterar3($id = FALSE) {
       ...
       # $this->form_validation->set_rules('ValorVenda', 'Valor do Convênio', 'required|trim');
        ...
        $data['form_open_path'] = 'prodaux32/alterar3';
        $data['readonly'] = '';
        $data['disabled'] = '';
        $data['panel'] = 'primary';
        $data['metodo'] = 2;
