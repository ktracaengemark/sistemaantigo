...
class Despesas extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(array('Basico_model', 'Servico_model', 'Produto_model', 'Produtos_model', 'Despesas_model', 'Tipodespesa_model', 'Formapag_model', 'Cliente_model'));
...
*/
        ...
        if ($this->form_validation->run() === FALSE) {
			...
			$data['despesas']['idSis_Empresa'] = $_SESSION['log']['idSis_Empresa'];
            ...
            */
            ...
            if (isset($data['parcelaspag'])) {
                ...
                for($j=1;$j<=$max;$j++) {
                    ...
                    $data['parcelaspag'][$j]['idSis_Empresa'] = $_SESSION['log']['idSis_Empresa'];
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if (isset($data['parcelaspag']) || (!isset($data['parcelaspag']) && isset($data['update']['parcelaspag']['anterior']) ) ) {
                ...
                for($j=0;$j<$max;$j++) {
                    ...
                    $data['update']['parcelaspag']['inserir'][$j]['idSis_Empresa'] = $_SESSION['log']['idSis_Empresa'];
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        for ($i = 1; $i <= $data['count']['PRCount']; $i++) {
            ...
            if ($this->input->post('idSis_Empresa' . $i) || $this->input->post('idApp_Despesas' . $i) ||
					$this->input->post('ParcelaPagaveis' . $i) || $this->input->post('ValorParcelaPagaveis' . $i) || 
					$this->input->post('DataVencimentoPagaveis' . $i) || $this->input->post('ValorPagoPagaveis' . $i) || 
					$this->input->post('DataPagoPagaveis' . $i) || $this->input->post('QuitadoPagaveis' . $i)) {
                $data['parcelaspag'][$j]['idApp_ParcelasPagaveis'] = $this->input->post('idApp_ParcelasPagaveis' . $i);
                $data['parcelaspag'][$j]['idSis_Empresa'] = $this->input->post('idSis_Empresa' . $i);
				$data['parcelaspag'][$j]['idApp_Despesas'] = $this->input->post('idApp_Despesas' . $i);
				$data['parcelaspag'][$j]['ParcelaPagaveis'] = $this->input->post('ParcelaPagaveis' . $i);
                $data['parcelaspag'][$j]['ValorParcelaPagaveis'] = $this->input->post('ValorParcelaPagaveis' . $i);
                $data['parcelaspag'][$j]['DataVencimentoPagaveis'] = $this->input->post('DataVencimentoPagaveis' . $i);
                $data['parcelaspag'][$j]['ValorPagoPagaveis'] = $this->input->post('ValorPagoPagaveis' . $i);
        ...
        if ($id) {
           ...
           # $_SESSION['Cliente'] = $this->Cliente_model->get_cliente($data['despesas']['idApp_Cliente'], TRUE);
            ...
            #### App_ParcelasPagaveis ####
            $data['parcelaspag'] = $this->Despesas_model->get_parcelaspagalterar($id);
            if (count($data['parcelaspag']) > 0) {
                $data['parcelaspag'] = array_combine(range(1, count($data['parcelaspag'])), array_values($data['parcelaspag']));
				$data['count']['PRCount'] = count($data['parcelaspag']);
        ...
        $data['form_open_path'] = 'despesas/alterarparcela';
        $data['readonly'] = '';
        $data['disabled'] = '';
        $data['panel'] = 'primary';
        $data['metodo'] = 2;
...
*/
        ...
        #run form validation
        if ($this->form_validation->run() === FALSE) {
            $this->load->view('despesas/form_despesasalterarparcela', $data);
        } else {
            ...
            #### App_ParcelasRec ####
            $data['update']['parcelaspag']['anterior'] = $this->Despesas_model->get_parcelaspagalterar($data['parcelaspag']['idSis_Empresa']);
            if (isset($data['parcelaspag']) || (!isset($data['parcelaspag']) && isset($data['update']['parcelaspag']['anterior']) ) ) {
                ...
                for($j=0;$j<$max;$j++) {
                    ...
                    $data['update']['parcelaspag']['inserir'][$j]['idSis_Empresa'] = $_SESSION['log']['idSis_Empresa'];
					$data['update']['parcelaspag']['inserir'][$j]['idTab_Modulo'] = $_SESSION['log']['idTab_Modulo'];
                    $data['update']['parcelaspag']['inserir'][$j]['idApp_Despesas'] = $data['update']['parcelaspag']['inserir'][$j]['idApp_Despesas'];
                    $data['update']['parcelaspag']['inserir'][$j]['ValorParcelaPagaveis'] = str_replace(',', '.', str_replace('.', '', $data['update']['parcelaspag']['inserir'][$j]['ValorParcelaPagaveis']));
                    $data['update']['parcelaspag']['inserir'][$j]['DataVencimentoPagaveis'] = $this->basico->mascara_data($data['update']['parcelaspag']['inserir'][$j]['DataVencimentoPagaveis'], 'mysql');
                    $data['update']['parcelaspag']['inserir'][$j]['ValorPagoPagaveis'] = str_replace(',', '.', str_replace('.', '', $data['update']['parcelaspag']['inserir'][$j]['ValorPagoPagaveis']));
                    $data['update']['parcelaspag']['inserir'][$j]['DataPagoPagaveis'] = $this->basico->mascara_data($data['update']['parcelaspag']['inserir'][$j]['DataPagoPagaveis'], 'mysql');
                ...
                if (count($data['update']['parcelaspag']['inserir']))
                    $data['update']['parcelaspag']['bd']['inserir'] = $this->Despesas_model->set_parcelaspagalterar($data['update']['parcelaspag']['inserir']);

                if (count($data['update']['parcelaspag']['alterar']))
                    $data['update']['parcelaspag']['bd']['alterar'] =  $this->Despesas_model->update_parcelaspagalterar($data['update']['parcelaspag']['alterar']);

                if (count($data['update']['parcelaspag']['excluir']))
                    $data['update']['parcelaspag']['bd']['excluir'] = $this->Despesas_model->delete_parcelaspagalterar($data['update']['parcelaspag']['excluir']);
...
*/
    ...
    public function alterarparcelaBKP($id = FALSE) {
...
*/
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            #### App_ParcelasRec ####
            $data['update']['parcelaspag']['anterior'] = $this->Despesas_model->get_parcelaspagalterar($data['despesas']['idSis_Empresa']);
            if (isset($data['parcelaspag']) || (!isset($data['parcelaspag']) && isset($data['update']['parcelaspag']['anterior']) ) ) {
                ...
                for($j=0;$j<$max;$j++) {
                    $data['update']['parcelaspag']['inserir'][$j]['idSis_Usuario'] = $_SESSION['log']['id'];
                    $data['update']['parcelaspag']['inserir'][$j]['idSis_Empresa'] = $data['despesas']['idSis_Empresa'];
					$data['update']['parcelaspag']['inserir'][$j]['idTab_Modulo'] = $_SESSION['log']['idTab_Modulo'];
                    #$data['update']['parcelaspag']['inserir'][$j]['idApp_Despesas'] = $data['despesas']['idApp_Despesas'];
                    $data['update']['parcelaspag']['inserir'][$j]['ValorParcelaPagaveis'] = str_replace(',', '.', str_replace('.', '', $data['update']['parcelaspag']['inserir'][$j]['ValorParcelaPagaveis']));
                    $data['update']['parcelaspag']['inserir'][$j]['DataVencimentoPagaveis'] = $this->basico->mascara_data($data['update']['parcelaspag']['inserir'][$j]['DataVencimentoPagaveis'], 'mysql');
