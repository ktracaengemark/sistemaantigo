...
class Funcao extends CI_Controller {
    ...
    public function cadastrar3($tabela = NULL) {
        ...
        $data['titulo'] = 'Cadastrar Funcao';
        $data['form_open_path'] = 'funcao/cadastrar3';
        $data['readonly'] = '';
        $data['disabled'] = '';
        $data['panel'] = 'primary';
        $data['metodo'] = 1;
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if ($data['idTab_Funcao'] === FALSE) {
                ...
                $this->basico->erro($msg);
                $this->load->view('funcao/cadastrar3', $data);
            } else {
                ...
                redirect(base_url() . 'funcao/cadastrar3' . $data['msg']);
                exit();
            }
        }
    ...
    public function alterar3($id = FALSE) {
        ...
        $data['titulo'] = 'Editar Funcao';
        $data['form_open_path'] = 'funcao/alterar3';
        $data['readonly'] = '';
        $data['disabled'] = '';
        $data['panel'] = 'primary';
        $data['metodo'] = 2;
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if ($data['auditoriaitem'] && $this->Funcao_model->update_funcao($data['query'], $data['query']['idTab_Funcao']) === FALSE) {
                $data['msg'] = '?m=2';
                redirect(base_url() . 'funcao/alterar3/' . $data['msg']);
                exit();
            } else {
