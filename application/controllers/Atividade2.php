...
class Atividade2 extends CI_Controller {
    ...
    public function cadastrar3($tabela = NULL) {
        ...
        $data['form_open_path'] = 'atividade2/cadastrar3';
        $data['readonly'] = '';
        $data['disabled'] = '';
        $data['panel'] = 'primary';
        $data['metodo'] = 1;
        ...
        $data['list'] = $this->load->view('atividade/list_atividade3', $data, TRUE);

        #run form validation
        if ($this->form_validation->run() === FALSE) {
            $this->load->view('atividade/pesq_atividade3', $data);
        } else {
            ...
            if ($data['idApp_Atividade'] === FALSE) {
                ...
                $this->load->view('atividade/cadastrar3', $data);
            } else {
                ...
                redirect(base_url() . 'atividade2/cadastrar3' . $data['msg']);
                exit();
            }
        }
    ...
    public function alterar3($id = FALSE) {
        ...
        $data['form_open_path'] = 'atividade2/alterar3';
        $data['readonly'] = '';
        $data['disabled'] = '';
        $data['panel'] = 'primary';
        $data['metodo'] = 2;
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if ($data['auditoriaitem'] && $this->Atividade_model->update_atividade($data['query'], $data['query']['idApp_Atividade']) === FALSE) {
                $data['msg'] = '?m=2';
                redirect(base_url() . 'atividade2/alterar3/' . $data['msg']);
                exit();
            } else {
