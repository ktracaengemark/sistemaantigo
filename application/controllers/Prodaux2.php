...
class Prodaux2 extends CI_Controller {
    ...
    public function alterarlogo($id = FALSE) {
        ...
        $data['form_open_path'] = 'prodaux2/alterarlogo';
        ...
        if ($this->form_validation->run() === FALSE) {
            #load login view
            $this->load->view('prodaux2/form_perfil2', $data);
        }
        else {
            ...
            if (!$this->upload->do_upload('Arquivo')) {
                ...
                if ($data['idSis_Arquivo'] === FALSE) {
					...
					if ($data['auditoriaitem'] && $this->Prodaux2_model->update2_prodaux2($data['query'], $data['query']['idTab_Prodaux2']) === FALSE) {
						$data['msg'] = '?m=2';
						redirect(base_url() . 'prodaux2/form_perfil2/' . $data['query']['idTab_Prodaux2'] . $data['msg']);
						exit();
					} else {
