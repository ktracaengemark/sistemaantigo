        ...
        if ($id) {
            ...
            if (count($data['orcatrata']) > 0) {
				...
				if (isset($data['orcatrata'])) {
					/*
					foreach ($data['orcatrata'] as $value) {
							
						echo "<pre>";
						print_r($value['idApp_OrcaTrata']);
						echo "</pre>";
						
						#### App_Parcelas####
						$data['parcelasrec'] = $this->Orcatrataprintcobranca_model->get_parcelasrec($value['idApp_OrcaTrata']);
						...
						if (count($data['parcelasrec']) > 0) {
							...
							if (isset($data['parcelasrec'])) {
								...
								for($k=1; $k <= $data['count']['PRCount']; $k++) {
									$data['parcelasrec'][$k]['DataVencimento'] = $this->basico->mascara_data($data['parcelasrec'][$k]['DataVencimento'], 'barras');
									$data['parcelasrec'][$k]['DataPago'] = $this->basico->mascara_data($data['parcelasrec'][$k]['DataPago'], 'barras');
								}
							}
						}						
