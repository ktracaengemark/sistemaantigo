...
class Relatoriomatriz_model extends CI_Model {
    ...
    public function list_financeiro($data, $completo) {
        ...
        */
		...
		$data['NomeEmpresa'] = ($data['NomeEmpresa']) ? ' AND C.idApp_Cliente = ' . $data['NomeEmpresa'] : FALSE;
	...
	public function list_receitas($data, $completo) {
        ...
        $query = $this->db->query('
            SELECT
                E.NomeEmpresa,
                CONCAT(IFNULL(E.NomeEmpresa,""), " / ", IFNULL(OT.Receitas,""), " / ", IFNULL(TR.TipoReceita,"")) AS NomeEmpresa,
				TR.TipoReceita,
	...
	public function list_receitas2($data, $completo) {
...
exit();*/
	...
	public function list_rankingvendas($data) {
        ...
        $data['NomeEmpresa'] = ($data['NomeEmpresa']) ? ' AND TC.idApp_Cliente = ' . $data['NomeEmpresa'] : FALSE;
        $data['Campo'] = (!$data['Campo']) ? 'TC.NomeEmpresa' : $data['Campo'];
        $data['Ordenamento'] = (!$data['Ordenamento']) ? 'ASC' : $data['Ordenamento'];
        ####################################################################
        #LISTA DE CLIENTES
        $query['NomeEmpresa'] = $this->db->query(
            'SELECT
                TC.idApp_Cliente,
                CONCAT(IFNULL(TC.NomeEmpresa,"")) AS NomeEmpresa,
				TOT.ValorOrca
            FROM
                App_Cliente AS TC
        ...
        $query['NomeEmpresa'] = $query['NomeEmpresa']->result();
		...
		#$estoque = array();
        foreach ($query['NomeEmpresa'] as $row) {
            #echo $row->idTab_Produtos . ' # ' . $row->Produtos . '<br />';
            #$estoque[$row->idTab_Produtos] = $row->Produtos;
            $rankingvendas->{$row->idApp_Cliente} = new stdClass();
            $rankingvendas->{$row->idApp_Cliente}->NomeEmpresa = $row->NomeEmpresa;
        }
...
*/
    ...
    public function list_orcamento($data, $completo) {
        ...
        $query = $this->db->query('
            SELECT
                C.NomeEmpresa,
				C.Telefone1,
				CONCAT(IFNULL(C.NomeEmpresa,""), " --- ", IFNULL(C.Telefone1,"")) AS NomeEmpresa,
				OT.idApp_OrcaTrata,
                OT.AprovadoOrca,
                OT.DataOrca,
				OT.DataEntradaOrca,
	...
	public function list_clientes($data, $completo) {
        ...
        $data['Ordenamento'] = (!$data['Ordenamento']) ? 'ASC' : $data['Ordenamento'];
		$filtro10 = ($data['Ativo'] != '#') ? 'C.Ativo = "' . $data['Ativo'] . '" AND ' : FALSE;
        $q = ($_SESSION['log']['Permissao'] > 2) ? ' C.idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . ' AND ' : FALSE;
    ...
    public function select_cliente() {
        ...
        $query = $this->db->query('
            SELECT
                C.idApp_Cliente,
                CONCAT(IFNULL(C.NomeEmpresa, ""), " --- ", IFNULL(C.Telefone1, ""), " --- ", IFNULL(C.Telefone2, ""), " --- ", IFNULL(C.Telefone3, "")) As NomeEmpresa
            FROM
                App_Cliente AS C
        ...
        $array[0] = ':: Todos ::';
        foreach ($query->result() as $row) {
			$array[$row->idApp_Cliente] = $row->NomeEmpresa;
        }
	...
	public function select_orcatrata() {
        ...
        $query = $this->db->query('
            SELECT
                CONCAT(P.idApp_OrcaTrata, " - ",C.NomeEmpresa) AS idApp_OrcaTrata,
                P.idApp_Cliente,
				C.NomeEmpresa
            FROM
                App_OrcaTrata AS P
