...
class Relatorioempresamatriz_model extends CI_Model {
    ...
    public function list_financeiro($data, $completo) {
        ...
        */
		...
		$data['Nome'] = ($data['Nome']) ? ' AND C.idSis_UsuarioMatriz = ' . $data['Nome'] : FALSE;
        ...
        $query = $this->db->query('
            ...
            WHERE
                ...
                ' . $filtro2 . '
                ' . $filtro3 . '
                C.idSis_UsuarioMatriz = OT.idSis_UsuarioMatriz
                ' . $data['Nome'] . '
	...
	public function list_receitas($data, $completo) {
        ...
        if ($completo === FALSE) {
            ...
            foreach ($query->result() as $row) {
                ...
                #esse trecho pode ser melhorado, serve para somar apenas uma vez
                #o valor da entrada que pode aparecer mais de uma vez
                if ($ant != $row->idApp_OrcaTrataEmp) {
                    $ant = $row->idApp_OrcaTrataEmp;
                    $somaentrada += $row->ValorEntradaOrca;
                }
                else {
                    $row->ValorEntradaOrca = FALSE;
...
exit();*/
	...
	public function list_rankingvendas($data) {
        ...
        $data['Nome'] = ($data['Nome']) ? ' AND TC.idSis_UsuarioMatriz = ' . $data['Nome'] : FALSE;
        $data['Campo'] = (!$data['Campo']) ? 'TC.Nome' : $data['Campo'];
        $data['Ordenamento'] = (!$data['Ordenamento']) ? 'ASC' : $data['Ordenamento'];
        ####################################################################
        #LISTA DE CLIENTES
        $query['Nome'] = $this->db->query(
            'SELECT
                TC.idSis_UsuarioMatriz,
                CONCAT(IFNULL(TC.Nome,"")) AS Nome,
				TOT.ValorOrca
            FROM
                Sis_UsuarioMatriz AS TC
				LEFT JOIN App_OrcaTrata AS TOT ON TOT.idSis_UsuarioMatriz = TC.idSis_UsuarioMatriz
            WHERE
        ...
        $query['Orcamentos'] = $this->db->query(
            ...
            FROM
                App_OrcaTrata AS TOT
                    LEFT JOIN Sis_UsuarioMatriz AS TC ON TC.idSis_UsuarioMatriz = TOT.idSis_UsuarioMatriz
		...
		#$estoque = array();
        foreach ($query['Nome'] as $row) {
            #echo $row->idTab_Produtos . ' # ' . $row->Produtos . '<br />';
            #$estoque[$row->idTab_Produtos] = $row->Produtos;
            $rankingvendas->{$row->idSis_UsuarioMatriz} = new stdClass();
            $rankingvendas->{$row->idSis_UsuarioMatriz}->Nome = $row->Nome;
        }
...
exit();*/
        ...
        foreach ($query['Orcamentos'] as $row) {
            if (isset($rankingvendas->{$row->idSis_UsuarioMatriz}))
                $rankingvendas->{$row->idSis_UsuarioMatriz}->QtdOrcam = $row->QtdOrcam;
        }
		
		foreach ($query['Descontos'] as $row) {
            if (isset($rankingvendas->{$row->idSis_UsuarioMatriz}))
                $rankingvendas->{$row->idSis_UsuarioMatriz}->QtdDescon = $row->QtdDescon;
        }
		
		foreach ($query['Parcelas'] as $row) {
            if (isset($rankingvendas->{$row->idSis_UsuarioMatriz}))
                $rankingvendas->{$row->idSis_UsuarioMatriz}->QtdParc = $row->QtdParc;
        }
		
		foreach ($query['Devolucoes'] as $row) {
            if (isset($rankingvendas->{$row->idSis_UsuarioMatriz}))
                $rankingvendas->{$row->idSis_UsuarioMatriz}->QtdDevol = $row->QtdDevol;
        }
...
*/
	...
	public function list_servicosprest1($data, $completo) {
		...
		$query = $this->db->query('
            ...
            WHERE
                C.idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . ' AND
				C.idTab_Modulo = ' . $_SESSION['log']['idTab_Modulo'] . ' AND
				(' . $consulta . ') AND
		...
		$query = $this->db->query('
            ...
            FROM
				...
				App_OrcaTrata AS OT
					LEFT JOIN App_ServicoVenda AS PV ON PV.idApp_OrcaTrata = OT.idApp_OrcaTrata
					LEFT JOIN Tab_Servico AS PD ON PD.idTab_Servico = PV.idTab_Servico
					LEFT JOIN Sis_UsuarioMatriz AS TSU ON TSU.idSis_UsuarioMatriz = PV.idSis_UsuarioMatriz
					LEFT JOIN App_Profissional AS P ON P.idApp_Profissional = OT.ProfissionalOrca
            WHERE
                C.Empresa = ' . $_SESSION['log']['Empresa'] . ' AND
				C.idTab_Modulo = ' . $_SESSION['log']['idTab_Modulo'] . ' AND
        ...
        $query = $this->db->query('
            ...
            FROM
                App_Despesas AS TCO
					...
					LEFT JOIN Sis_UsuarioMatriz AS C ON C.idSis_UsuarioMatriz = TR.idSis_UsuarioMatriz
            WHERE
                TCO.Empresa = ' . $_SESSION['log']['Empresa'] . ' AND
    ...
    public function list_orcamentoempresa($data, $completo) {
        ...
        $data['Nome'] = ($data['Nome']) ? ' AND TSU.idSis_UsuarioMatriz = ' . $data['Nome'] : FALSE;
        ...
        $query = $this->db->query('
            SELECT
				...
				TFP.FormaPag,
				TSU.Nome,
				TSEF.idSis_EmpresaFilial
            FROM
                Sis_UsuarioMatriz AS TSU,
				App_OrcaTrata AS OT
				LEFT JOIN Sis_EmpresaFilial AS TSEF ON TSEF.idSis_EmpresaFilial = OT.idSis_EmpresaFilial
				LEFT JOIN Tab_FormaPag AS TFP ON TFP.idTab_FormaPag = OT.FormaPagamento

            WHERE
				TSEF.Empresa = ' . $_SESSION['log']['Empresa'] . ' AND
                (' . $consulta . ') AND
				(' . $consulta2 . ') AND
				(' . $consulta3 . ') AND
				(' . $consulta4 . ') AND
                ...
                ' . $filtro2 . '
				' . $filtro3 . '
                TSU.idSis_UsuarioMatriz = OT.idSis_UsuarioMatriz
                ' . $data['Nome'] . '
				' . $data['FormaPag'] . ' AND
				OT.TipoRD = "R"
            ORDER BY
	...
	public function list_devolucao1($data, $completo) {
        ...
        $query = $this->db->query('
            ...
            FROM
                Sis_UsuarioMatriz AS C,
                App_OrcaTrata AS OT
				LEFT JOIN Sis_UsuarioMatriz AS TSU ON TSU.idSis_UsuarioMatriz = OT.idSis_UsuarioMatriz
				LEFT JOIN Tab_FormaPag AS TFP ON TFP.idTab_FormaPag = OT.FormaPagamento
				LEFT JOIN Tab_TipoDevolucao AS TD ON TD.idTab_TipoDevolucao = OT.TipoDevolucao
            WHERE
				C.Empresa = ' . $_SESSION['log']['Empresa'] . ' AND
	...
	public function list_despesa($data, $completo) {
        ...
        $query = $this->db->query('
            ...
            WHERE
				...
				D.idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . ' AND
				OT.idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . ' AND
				(' . $consulta . ')
				' . $data['TipoDespesa'] . '
        ...
        $query = $this->db->query('
            ...
            FROM
                Sis_UsuarioMatriz AS C
					...
					LEFT JOIN App_ContatoUsuario AS CC ON C.idSis_UsuarioMatriz = CC.idSis_UsuarioMatriz
	...
	public function list_profissionais($data, $completo) {
        ...
        $query = $this->db->query('
            ...
            WHERE
                P.idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . ' AND
        ...
        /*
        #AND
        #P.idApp_Profissional = OT.idSis_UsuarioMatriz
	...
	public function list_funcionario($data, $completo) {
        ...
        $data['Nome'] = ($data['Nome']) ? ' AND F.idSis_UsuarioMatriz = ' . $data['Nome'] : FALSE;
        $data['Campo'] = (!$data['Campo']) ? 'F.idSis_UsuarioMatriz' : $data['Campo'];
        $data['Ordenamento'] = (!$data['Ordenamento']) ? 'ASC' : $data['Ordenamento'];

        $query = $this->db->query('
            SELECT
                F.idSis_UsuarioMatriz,
                F.Nome,
				F.DataCriacao,
				F.Sexo,
				F.CpfUsuario,
	...
	public function list_empresas($data, $completo) {
        ...
        $query = $this->db->query('
            ...
            WHERE
                E.idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . ' AND
	...
	public function list_tarefa($data, $completo) {
        ...
        $query = $this->db->query('
            ...
            WHERE
				...
				TF.idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . ' AND
    ...
    public function select_cliente() {
        ...
        $query = $this->db->query('
            SELECT
                C.idSis_UsuarioMatriz,
                CONCAT(IFNULL(C.Nome, ""), " --- ", IFNULL(C.Telefone1, ""), " --- ", IFNULL(C.Telefone2, ""), " --- ", IFNULL(C.Telefone3, "")) As Nome
            FROM
                Sis_UsuarioMatriz AS C
        ...
        $array[0] = ':: Todos ::';
        foreach ($query->result() as $row) {
			$array[$row->idSis_UsuarioMatriz] = $row->Nome;
        }
	...
	public function select_empresas() {
        ...
        $query = $this->db->query('
            ...
            FROM
                App_Empresa
            WHERE
                idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . ' AND
				idTab_Modulo = ' . $_SESSION['log']['idTab_Modulo'] . '
            ORDER BY
                NomeEmpresa ASC
        ');
	...
	public function select_profissional2() {
        ...
        $query = $this->db->query('
            ...
            WHERE
                P2.idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . ' AND
	...
	public function select_profissional3() {
        ...
        $query = $this->db->query('
            ...
            WHERE
                ...
                P.idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . '
	...
	public function select_obstarefa() {
        ...
        $query = $this->db->query('
            ...
            FROM
                App_Tarefa AS OB
            WHERE
                OB.idSis_UsuarioMatriz = ' . $_SESSION['log']['id'] . ' AND
	...
	public function select_produtosemp() {
	...
	public function select_orcatrata() {
        ...
        $query = $this->db->query('
            SELECT
                CONCAT(P.idApp_OrcaTrata, " - ",C.Nome) AS idApp_OrcaTrata,
                P.idSis_UsuarioMatriz,
				C.Nome
            FROM
                App_OrcaTrata AS P
				LEFT JOIN Sis_UsuarioMatriz AS C ON C.idSis_UsuarioMatriz = P.idSis_UsuarioMatriz
            WHERE
                P.Empresa = ' . $_SESSION['log']['Empresa'] . ' AND
				P.idTab_Modulo = ' . $_SESSION['log']['idTab_Modulo'] . '
            ORDER BY
	...
	public function select_usuarioemp() {
        ...
        $array[0] = ':: Todos ::';
        foreach ($query->result() as $row) {
            $array[$row->idSis_UsuarioMatriz] = $row->NomeUsuario;
        }
