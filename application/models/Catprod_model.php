...
class Catprod_model extends CI_Model {
        ...
        $query = $this->db->query('
			...
			FROM 
				Tab_Catprod AS TP
				 ...
				 LEFT JOIN Tab_Atributo AS TV ON TV.idTab_Catprod = TP.idTab_Catprod
    ...
    public function delete_catprod($id) {
		...
		$query = $this->db->delete('Tab_Opcao', array('idTab_Catprod' => $id));
        $query = $this->db->delete('Tab_Atributo', array('idTab_Catprod' => $id));
