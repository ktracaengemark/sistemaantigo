...
class Relatoriocli_model extends CI_Model {
...
*/
	...
	public function list_servicosprest1($data, $completo) {
		...
		$query = $this->db->query('
            ...
            WHERE
                C.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' AND
				C.idTab_Modulo = ' . $_SESSION['log']['idTab_Modulo'] . ' AND
				(' . $consulta . ') AND
		...
		$query = $this->db->query('
            ...
            FROM
				...
				App_OrcaTrata AS OT
					LEFT JOIN App_ServicoVenda AS PV ON PV.idApp_OrcaTrata = OT.idApp_OrcaTrata
					LEFT JOIN Tab_Servico AS PD ON PD.idTab_Servico = PV.idTab_Servico
					LEFT JOIN Sis_UsuarioCli AS TSU ON TSU.idSis_UsuarioCli = PV.idSis_UsuarioCli
					LEFT JOIN App_Profissional AS P ON P.idApp_Profissional = OT.ProfissionalOrca
            WHERE
                C.Empresa = ' . $_SESSION['log']['Empresa'] . ' AND
				C.idTab_Modulo = ' . $_SESSION['log']['idTab_Modulo'] . ' AND
    ...
    public function list_orcamento($data, $completo) {
        ...
        $query = $this->db->query('
            ...
            FROM
                App_Cliente AS C,
                App_OrcaTrata AS OT
				LEFT JOIN Sis_UsuarioCli AS TSU ON TSU.idSis_UsuarioCli = OT.idSis_UsuarioCli
				LEFT JOIN Tab_FormaPag AS TFP ON TFP.idTab_FormaPag = OT.FormaPagamento
	...
	public function list_despesa($data, $completo) {
        ...
        $query = $this->db->query('
            ...
            WHERE
				...
				D.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' AND
				OT.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' AND
				(' . $consulta . ')
				' . $data['TipoDespesa'] . '
	...
	public function list_clientes($data, $completo) {
        ...
        $data['Ordenamento'] = (!$data['Ordenamento']) ? 'ASC' : $data['Ordenamento'];
		$filtro10 = ($data['Ativo'] != '#') ? 'C.Ativo = "' . $data['Ativo'] . '" AND ' : FALSE;
        $q = ($_SESSION['log']['Permissao'] > 2) ? ' C.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' AND ' : FALSE;
	...
	public function list_associado($data, $completo) {
        ...
        $data['Nome'] = ($data['Nome']) ? ' AND C.idSis_UsuarioCli = ' . $data['Nome'] : FALSE;
	...
	public function list_profissionais($data, $completo) {
        ...
        $query = $this->db->query('
            ...
            WHERE
                P.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' AND
	...
	public function list_funcionario($data, $completo) {
        ...
        $data['Nome'] = ($data['Nome']) ? ' AND F.idSis_UsuarioCli = ' . $data['Nome'] : FALSE;
	...
	public function list_empresas1($data, $completo) {
        ...
        $query = $this->db->query('
            ...
            WHERE
                E.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' AND
				E.idTab_Modulo = ' . $_SESSION['log']['idTab_Modulo'] . '
				' . $data['NomeEmpresa'] . '
			ORDER BY
                ' . $data['Campo'] . ' ' . $data['Ordenamento'] . '
	...
	public function list_tarefa($data, $completo) {
        ...
        $query = $this->db->query('
            ...
            WHERE
				...
				TF.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' AND
	...
	public function list_procedimento($data, $completo) {
		...
		$query = $this->db->query('
            ...
            WHERE
                C.idTab_Modulo = ' . $_SESSION['log']['idTab_Modulo'] . ' AND
				C.idSis_Empresa = "0" AND
				C.idApp_OrcaTrataCli = "0" AND
				C.idApp_Cliente = "0" AND
				' . $filtro10 . '
				C.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' 
                ' . $data['Dia'] . ' 
				' . $data['Mesvenc'] . ' 
				' . $data['Ano'] . ' 
	...
	public function select_associado() {
        ...
        $array[0] = ':: Todos ::';
        foreach ($query->result() as $row) {
			$array[$row->idSis_UsuarioCli] = $row->Nome;
        }
	...
	public function select_profissional2() {
        ...
        $query = $this->db->query('
            ...
            WHERE
                P2.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' AND
	...
	public function select_profissional3() {
        ...
        $query = $this->db->query('
            ...
            WHERE
                ...
                P.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . '
	...
	public function select_obstarefa() {
        ...
        $query = $this->db->query('
            ...
            FROM
                App_Tarefa AS OB
            WHERE
                OB.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' AND
	...
	public function select_usuario() {
        ...
        $array[0] = ':: Todos ::';
        foreach ($query->result() as $row) {
            $array[$row->idSis_UsuarioCli] = $row->NomeUsuario;
        }
