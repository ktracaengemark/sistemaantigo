...
$query = ($_SESSION['log']['NomeUsuario'] && isset($_SESSION['log']['NomeUsuario'])) ?
    #'P.idSis_UsuarioCli = ' . $_SESSION['log']['NomeUsuario'] . ' AND ' : FALSE;
	'A.idSis_UsuarioCli = ' . $_SESSION['log']['NomeUsuario'] . ' AND ' : FALSE;

$permissao = ($_SESSION['log']['Permissao'] > 2) ?
	'A.idSis_UsuarioCli = ' . $_SESSION['log']['id'] . ' AND ' : FALSE;
...
while ($row = mysql_fetch_assoc($result)) {
    ...
    if ($row['Evento']) {
		...
		#$title = utf8_encode($row['idSis_UsuarioCli']);
        ...
        if ($row['Paciente'] == 'D') {
			...
			#$profissional = utf8_encode($row['NomeUsuario']);
			#$profissional = utf8_encode($row['idApp_Agenda']);
			#$profissional = utf8_encode($row['idSis_UsuarioCli']);
            $profissional = mb_convert_encoding($row['NomeProfissional'], "UTF-8", "ISO-8859-1");
    ...
    $url = 'consultacli/alterar' . $c . '/' . $row['idApp_Consulta'];
    ...
    if ($row['DataFim'] < date('Y-m-d H:i:s')) {
        ...
        //$url = 'consultacli/alterar/'.$row['idApp_Paciente'].'/'.$row['idApp_Consulta'];
        $textColor = 'black';
